...
document.addEventListener("DOMContentLoaded", function() {
    ...
    keyStore.open().
    then(function() {
        var getData = keyStore.getKey("name", "rsaSelf");
        getData.then( function(keyPair) {
    ...
    function createAndSaveAKeyPair() {
        ...
        then(function (key) {
            //keyPair = key;
            keyStore.saveRsaKey(key.publicKey, key.privateKey, "rsaSelf");
            return key;
        });
    }
        ...
        function processTheFile() {
            ...
            getData.then( function(keyPair) {
                ...
                function encrypt(plaintext, publicKey) {
                    ...
                    //var getAesData = keyStore.getKey("name", "aesRoom1");
                    //    getAesData.then( function(aesKey) {
                    //        if (typeof aesKey !== 'undefined') {
                    //            key = aesKey.encryptedKey;
                    //        }
                    //    });
                    ...
                    function packageResults(encryptedKey) {
                        ...
                        getAesData.then( function(aesKey) {
                            if (typeof aesKey === 'undefined') {
                                keyStore.saveAesKey(encryptedKey, "aesRoom1");
                            }
                        });
                        return new Blob(
                            [
    ...
    function decryptTheFile() {
        ...
        function processTheFile() {
            ...
            var iv = new Uint8Array( data, 0,  16);
            var ciphertext = new Uint8Array( data, 16);

            var getAesData = keyStore.getKey("name", "aesRoom1");
            getAesData.then( function(aesKey) {

                var encryptedKey = new Uint8Array(aesKey.encryptedKey);
